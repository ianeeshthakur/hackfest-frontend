// lib/aiAgentLogic.js
import { IMPACT_ORDERS } from "@/lib/data/impactOrders";
...
export function generateAIResponse(query, pathname, role, disruptionContext) {
  ...
  // ==========================================
  const { parseIntentAndParams, simulateScenario } = require('./scenarioEngine');
  const { isScenario, params: scenarioParams } = parseIntentAndParams(q);

  if (isScenario && !isIdle) {
  ...
  // ==========================================
  if (pathname.includes("/impact")) {
    const ordersList = role === "buyer" ? IMPACT_ORDERS.filter(o => o.buyer.includes("Zara")) : IMPACT_ORDERS;
    const numOrders = ordersList.length;
    
    if (q.includes("what's at risk") || q.includes("what is at risk") || (q.includes("what") && q.includes("risk"))) {
      const orderIds = ordersList.map(o => o.id).join(", ");
      return { 
        type: "text", 
        text: `${numOrders} orders are at risk: ${orderIds}. They are currently experiencing delays and require attention.`,
        followUps: ["How many units are affected?", "Which order should be prioritized?"]
      };
    }
    
    if (q.includes("units") || q.includes("how many") || q.includes("quantity")) {
      const totalUnitsStr = ordersList.reduce((acc, o) => acc + parseInt(o.qty.replace(/[^0-9]/g, '')), 0).toLocaleString();
      return {
        type: "text",
        text: `There are ${totalUnitsStr} units currently affected across ${numOrders} active orders.`,
        followUps: ["What's at risk?", "Which order should be prioritized?"]
      };
    }
    
    if (q.includes("prioritize") || q.includes("priority") || q.includes("first")) {
      const highestRisk = [...ordersList].sort((a, b) => b.risk - a.risk)[0];
      return {
        type: "text",
        text: `${highestRisk.id} has the highest risk score at ${highestRisk.risk}/100 and should be prioritized. It involves ${highestRisk.qty} of ${highestRisk.product} for ${highestRisk.buyer}, and is currently delayed by ${highestRisk.delay}.`,
        followUps: ["What's at risk?", "How many units are affected?"]
      };
    }

    // Context-aware fallback for impact page
    const highestRisk = [...ordersList].sort((a, b) => b.risk - a.risk)[0];
    return {
      type: "text",
      text: `Based on the current order data, ${numOrders} orders are delayed. ${highestRisk.id} is the most critical with a risk score of ${highestRisk.risk}. Let me know if you want to know what's at risk or how many units are affected.`
    };
  }
