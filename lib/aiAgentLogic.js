...
export function generateAIResponse(query, pathname, role, disruptionContext) {
  ...
  if (!isIdle) {
    if (currentConfig.id === "power") rootCause = "Grid power failure in Sector 4";
    if (currentConfig.id === "machine") rootCause = "Machine breakdown in the Garmenting unit";
    if (currentConfig.id === "worker") rootCause = "Severe worker shortage (25% absenteeism)";
    if (currentConfig.id === "material") rootCause = "Raw material delay from Supplier T-4";
