...
export function generateAIResponse(query, pathname, role, disruptionContext) {
  ...
  if (pathname.includes("/factory-owner")) {
    if (q.includes("health") || q.includes("status")) {
      if (isIdle) return { type: "text", text: "Your factory is currently operating at 100% capacity. Machine uptime is stable, and power supply is uninterrupted. All orders are on track." };
      
      return { type: "text", text: `A disruption has been detected. Operational capacity has fallen due to ${getCauseText()}. ${affectedOrders} orders are now exposed to delivery risk.` };
    }
    if (q.includes("risk") || q.includes("order")) {
      ...
      return { type: "text", text: `The highest risk is associated with ${affectedOrders} active orders approaching their delivery tolerance due to the current ${getCauseText()}.` };
    }
  ...
  if (pathname.includes("/command-center")) {
    if (q.includes("what happened") || q.includes("propagate")) {
      ...
      if (role === "owner") {
        if (currentConfig.type === "powerCut") return { type: "text", text: `A grid power failure in Sector 4 triggered an immediate shutdown of primary weaving machines. Production capacity dropped to ${Math.round((currentConfig.availableUnits/currentConfig.totalUnits)*100)}%, delaying ${affectedOrders} downstream orders by ${delay} hours.` };
        ...
        if (currentConfig.type === "workerShortage") return { type: "text", text: `A sudden 25% drop in workforce availability reduced manual operation speed, cascading into a ${delay}-hour delay for ${affectedOrders} active orders.` };
        ...
        if (currentConfig.type === "logisticsDelay") return { type: "text", text: `A logistics route delay was detected in transit. This creates a projected ${delay}-hour delay.` };
      } else {
